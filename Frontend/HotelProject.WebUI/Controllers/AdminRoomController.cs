...
namespace HotelProject.WebUI.Controllers
{
    public class AdminRoomController : Controller
    {
        ...
        public async Task<IActionResult> AddRoom(CreateRoomDto createRoomDto)
        {
            ...
            var responseMessage = await client.PostAsync("http://localhost:5278/api/Room", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"http://localhost:5278/api/Room/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"http://localhost:5278/api/Room/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                ...
                var values = JsonConvert.DeserializeObject<UpdateRoomDto>(jsonData["result"].ToString());
                return View(values);
            }
            return View();
        }
        ...
        public async Task<IActionResult> UpdateRoom(UpdateRoomDto updateRoomDto)
        {
            ...
            var responseMessage = await client.PutAsync("http://localhost:5278/api/Room/", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
